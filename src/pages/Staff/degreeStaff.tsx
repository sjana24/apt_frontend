...
export function StaffDegrees() {
  ...
  useEffect(() => {
    const fetchDegrees = async () => {
      ...
      try {
        ...
        const response = await degreeService.getDegreesByStaff(
          parseInt(staffId),
        );
        ...
        const uniqueDegrees = Array.from(
          new Map(
            assignments
              ...
              .map((a: any) => [
                a.module_details.degree_details.id,
                a.module_details.degree_details,
              ]),
          ).values(),
        ) as Degree[];
  ...
  const columns: Column<Degree>[] = [
    { key: "degreeProgram", header: "Program" },
    {
      key: "level",
      header: "Level",
      render: (item) => <Badge variant="secondary">Level {item.level}</Badge>,
    },
    {
      key: "semester",
      header: "Semester",
